 ...
 * turnRandom()
 *
 * Turn a random direction a random amount
 *
 * returns amount turned
*/
int turnRandom(int minDegrees, int maxDegrees) {

  //I probally don't want srand here
  srand(time(NULL));
  
  
  int r = rand() % 2; //some random number between 0 and 1
  
  int degrees = minDegrees + (rand() % (maxDegrees-minDegrees));
